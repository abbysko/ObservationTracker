...
(function () {
  ...
  function hasDuplicateSessionName(sessions, candidateName, excludeId) {
    ...
    return (Array.isArray(sessions) ? sessions : []).some(
      (entry) =>
        ...
        normalizeNameKey(entry?.sessionName || '') === targetKey
    );
  }
  ...
  if (window._otTrackSaveHandler) {
    window.removeEventListener(
      'ot-active-session-save-request',
      window._otTrackSaveHandler
    );
  }

  window._otTrackSaveHandler = async () => {
